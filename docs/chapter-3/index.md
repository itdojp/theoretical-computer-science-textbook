...
ε-閉包はゼロ回の ε 遷移も許すため、常に \\(q \\in ECLOSE(q)\\) である。ε-NFA \\(M=(Q,Σ,δ,q_0,F)\\) に対して、次の5つ組を持つ通常のNFA \\(M'=(Q,Σ,δ',q_0,F')\\) を構成する。

- \\(δ'(q,a)=ECLOSE(\\bigcup_{r \\in ECLOSE(q)}δ(r,a))\\)（\\(q \\in Q, a \\in Σ\\)）
