...
- Qᴅ = P(Qₙ)（N の状態集合の冪集合）
- q₀ᴅ = {q₀}
- Fᴅ = \\(\{R \subseteq Qₙ \mid R \cap Fₙ \ne \emptyset\}\\)
- δᴅ(R, a) = ⋃q∈R δₙ(q, a)

帰納法により、δ̂ᴅ({q₀}, w) = δ̂ₙ(q₀, w) が示される。
したがって、L(D) = L(N)。□

**注意**：状態数は最悪の場合指数的に増加する（\\(\lvert Qᴅ\rvert \le 2^{\lvert Qₙ\rvert}\\)）。
...
s = 0ᵖ1ᵖ を考える。\\(\lvert s\rvert = 2p \ge p\\) なので、s = xyz と分解できる。

条件 \\(\lvert xy\rvert \le p\\) より、y は 0 のみからなる。y = 0ᵏ（k > 0）とする。
...
{: #thm-3-12 }
1. **所属問題**：w ∈ L?（CYKアルゴリズム：O(n³\\(\lvert G\rvert\\))）
2. **空問題**：L = ∅?
3. **有限性問題**：L は有限？
...
1. 以下の言語を認識する DFA を構成せよ：
   (a) {w ∈ {0,1}* \\(\mid\\) w は 00 を部分文字列として含まない}
   (b) {w ∈ {a,b}* \\(\mid\\) w 中の a の個数は 3 の倍数}
   (c) {w ∈ {0,1}* \\(\mid\\) w を2進数と見たとき 3 で割り切れる}
...
8. Myhill–Nerode の定理を用いて、以下の言語の最小DFAの状態数の下界（ひいては最小状態数）を示せ：
   (a) L₁ = { w ∈ {a,b}* \\(\mid\\) w が部分文字列 "aba" を含む }
       提示: 接頭辞 ε, a, ab, aba が互いに識別可能であることを示す → 4 状態が必要
   (b) L₂ = { w ∈ {0,1}* \\(\mid\\) #1(w) ≡ 0 (mod 3) かつ最終文字が 0 }
       提示: 3（1 の剰余）×2（最終文字）= 6 通りの右コンテキストを区別する必要がある

   (c) L₃ = { w ∈ {0,1}* \\(\mid\\) 二進数としての値が 5 の倍数 }
       提示: 5 つの剰余類（mod 5）の右コンテキストが必要 ⇒ 下界 5
   (d) L₄ = { w ∈ {0,1}* \\(\mid\\) 二進数としての値が 7 の倍数 }
