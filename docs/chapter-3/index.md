...
- wᵢ：w の i 番目の文字（\\(1 \le i \le \lvert w\rvert\\)）
- \\(\Sigma^*\\)：Σ 上のすべての文字列の集合
- \\(\Sigma^+\\)：Σ 上の空でない文字列の集合（\\(\Sigma^+ = \Sigma^* \setminus \\{\epsilon\\}\\)）
- \\(\\Sigma^n\\)：長さ n の文字列の集合

**定義 3.3** Σ 上の**言語**（language）は、\\(\Sigma^*\\) の部分集合である。
{: #def-3-3 }
...
4. **部分文字列**：v が w の部分文字列 ⟺ \\(\exists x, y \in \Sigma^*,\ w = xvy\\)
   - **接頭辞**（prefix）：w = xv となる x
   - **接尾辞**（suffix）：w = vx となる x

### 3.1.3 言語の演算

**定義 3.5** 言語 L₁, L₂ ⊆ \\(\Sigma^*\\) に対する演算は次のとおりである。
{: #def-3-5 }

1. **和集合**：\\(L₁ \cup L₂ = \\{w \mid w \in L₁ \lor w \in L₂\\}\\)
2. **共通部分**（intersection）：\\(L₁ \cap L₂ = \\{w \mid w \in L₁ \land w \in L₂\\}\\)
3. **補集合**：\\(\overline{L} = \Sigma^* \setminus L\\)
4. **連結**：\\(L₁L₂ = \\{xy \mid x \in L₁ \land y \in L₂\\}\\)
5. **冪乗**
   - \\(L^0 = \\{\\epsilon\\}\\)
   - \\(L^{n+1} = L^n L\\)
6. **クリーネ閉包**：\\(L^* = \\bigcup_{n=0}^{\\infty} L^n\\)
7. **正閉包**：\\(L^+ = \\bigcup_{n=1}^{\\infty} L^n\\)

**例 3.1** L = {0, 11} のとき、次のとおりである。
{: #ex-3-1 }
- \\(L^2 = \\{00, 011, 110, 1111\\}\\)
- \\(L^* = \\{\\epsilon, 0, 11, 00, 011, 110, 1111, \\ldots\\}\\)
...
**定義 3.7** DFA M の**拡張遷移関数** δ̂: Q × \\(\Sigma^*\\) → Q を帰納的に定義する。
{: #def-3-7 }
- δ̂(q, ε) = q
- δ̂(q, wa) = δ(δ̂(q, w), a)（\\(w \in \Sigma^*, a \in \Sigma\\)）

**定義 3.8** DFA M が文字列 w を**受理する**とは、δ̂(q₀, w) ∈ F であること。
{: #def-3-8 }
M が認識する言語：\\(L(M) = \\{w \in \Sigma^* \mid \hat{\delta}(q₀, w) \in F\\}\\)
...
**定義 3.10** NFA M の拡張遷移関数 δ̂: Q × \\(\Sigma^*\\) → P(Q) を次のように定義する。
{: #def-3-10 }
- δ̂(q, ε) = {q}
...
*証明*：L を認識する DFA を M = (Q, Σ, δ, q₀, F) とし、\\(p = \lvert Q\rvert\\) とする。
\\(\lvert s\rvert \ge p\\) なる \\(s \in L\\) を考える。s = a₁a₂...aₙ（n ≥ p）とする。
...
**定義 3.24** PDA の**瞬時記述**（ID）：\\((q, w, \alpha) \in Q \times \Sigma^* \times \Gamma^*\\)
{: #def-3-24 }
- q：現在の状態
- w：未読の入力
- α：スタックの内容（左端がトップ）
...
**定義 3.26** PDAの2つの受理方式：
{: #def-3-26 }
1. **最終状態による受理**：\\(L(P) = \\{w \mid (q₀, w, Z₀) \vdash^{*} (q, \epsilon, \alpha),\ q \in F\\}\\)
2. **空スタックによる受理**：\\(N(P) = \\{w \mid (q₀, w, Z₀) \vdash^{*} (q, \epsilon, \epsilon)\\}\\)
...
**例 3.11** \\(L₁ = \\{a^{n}b^{n}c^{m} \mid n, m \ge 0\\}\\) と \\(L₂ = \\{a^{m}b^{n}c^{n} \mid n, m \ge 0\\}\\) は文脈自由言語だが、\\(L₁ \cap L₂ = \\{a^{n}b^{n}c^{n} \mid n \ge 0\\}\\) は文脈自由言語でない。
{: #ex-3-11 }
...
1. 以下の言語を認識する DFA を構成せよ：
   (a) \\(\\{w \\in \\{0,1\\}^* \\mid w \\text{ は部分文字列 } 00 \\text{ を含まない}\\}\\)
   (b) \\(\\{w \\in \\{a,b\\}^* \\mid w \\text{ 中の } a \\text{ の個数は } 3 \\text{ の倍数}\\}\\)
   (c) \\(\\{w \\in \\{0,1\\}^* \\mid w \\text{ を2進数と見たとき } 3 \\text{ で割り切れる}\\}\\)
...
8. Myhill–Nerode の定理を用いて、以下の言語の最小DFAの状態数の下界（ひいては最小状態数）を示せ：
   (a) \\(L_1 = \\{ w \\in \\{a,b\\}^* \\mid w \\text{ が部分文字列 } aba \\text{ を含む }\\}\\)
       提示: 接頭辞 ε, a, ab, aba が互いに識別可能であることを示す → 4 状態が必要
   (b) \\(L_2 = \\{ w \\in \\{0,1\\}^* \\mid w \\text{ 中の } 1 \\text{ の個数は } 3 \\text{ の倍数で、最終文字が } 0 \\}\\)
       提示: 3（1 の剰余）×2（最終文字）= 6 通りの右コンテキストを区別する必要がある

   (c) \\(L_3 = \\{ w \\in \\{0,1\\}^* \\mid w \\text{ を二進数として解釈した値が } 5 \\text{ の倍数 }\\}\\)
       提示: 5 つの剰余類（mod 5）の右コンテキストが必要 ⇒ 下界 5
   (d) \\(L_4 = \\{ w \\in \\{0,1\\}^* \\mid w \\text{ を二進数として解釈した値が } 7 \\text{ の倍数 }\\}\\)
