...
**定義 3.7** DFA M の**拡張遷移関数** δ̂: Q × \\(\Sigma^{\ast}\\) → Q を帰納的に定義する。
{: #def-3-7 }
- \\(δ̂(q, ε) = q\\)
- δ̂(q, wa) = δ(δ̂(q, w), a)（\\(w \in \Sigma^{\ast}, a \in \Sigma\\)）

**定義 3.8** DFA \\(M\\) が文字列 \\(w\\) を**受理する**とは、\\(\hat{\delta}(q_0, w) \in F\\) であること。
{: #def-3-8 }
M が認識する言語：\\(L(M) = \\{w \in \Sigma^{\ast} \mid \hat{\delta}(q₀, w) \in F\\}\\)
...
**定義 3.26** PDAの2つの受理方式：
{: #def-3-26 }
1. **最終状態による受理**：\\(L(P) = \\{w \mid (q₀, w, Z₀) \vdash^{\ast} (q, \epsilon, \alpha),\ q \in F\\}\\)
2. **空スタックによる受理**：\\(N(P) = \\{w \mid (q₀, w, Z₀) \vdash^{\ast} (q, \epsilon, \epsilon)\\}\\)
