...
**定義 5.2** 全入力で停止する決定性チューリング機械 M の**時間複雑度**（time complexity）\\(t_M: \mathbb{N} \to \mathbb{N}\\) を
{: #def-5-2 }
\\(t_M(n) = \max\{\text{M が長さ } n \text{ の入力 } w \text{ で停止するまでのステップ数}\}\\)
と定義する。全入力停止を仮定しない認識器については、最大値を \\(\mathbb{N} \cup \{\infty\}\\) に拡張して扱う。
...
1. <span id="exq-ch5-001"></span>以下の包含関係を証明せよ：
   \\((a) TIME(n) ⊂ TIME(n^2)\\)
   \\((b) L ⊆ NL ⊆ P\\)
   \\((c) NP ⊆ PSPACE\\)

- **詳細解答**（項目: (c)）: [付録Cの対応解答](../appendices/c/#ex-sol-ch5-001)

2. <span id="exq-ch5-002"></span>以下の問題が NP に属することを示せ：
   (a) グラフ同型問題
   (b) 整数計画問題
   (c) 最短ベクトル問題

3. <span id="exq-ch5-003"></span>3-SAT から以下への多項式時間還元を構成せよ：
   (a) 3-彩色問題
   (b) 頂点被覆問題
...
- **詳細解答**（項目: (b)）: [付録Cの対応解答](../appendices/c/#ex-sol-ch5-002)
- **詳細解答**（項目: (c)）: [付録Cの対応解答](../appendices/c/#ex-sol-ch5-003)
- **詳細解答**（項目: (f)）: [付録Cの対応解答](../appendices/c/#ex-sol-ch5-005)

4. <span id="exq-ch5-004"></span>PSPACE = NPSPACE を Savitch の定理を用いて証明せよ。

### 発展問題

5. <span id="exq-ch5-005"></span>時間階層定理の証明において、対角化言語が求める性質を持つことを詳細に証明せよ。

6. <span id="exq-ch5-006"></span>Cook-Levin の定理の証明で、チューリング機械の計算を論理式で符号化する際の具体的な構成を説明せよ。

- **詳細解答**: [付録Cの対応解答](../appendices/c/#ex-sol-ch5-004)

7. <span id="exq-ch5-007"></span>多項式階層が \\(\\Sigma_k^{\\mathrm{P}}\\) で崩壊したとき、実際にすべての上位レベルが \\(\\Sigma_k^{\\mathrm{P}}\\) と等しくなることを証明せよ。

8. <span id="exq-ch5-008"></span>BPP における誤り確率の増幅技法（amplification）を詳細に解析せよ。

### 探究課題

9. <span id="exq-ch5-009"></span>P vs NP 問題に関する既知の障壁（相対化、自然な証明、代数化）について調査し、なぜこれらが問題解決を困難にするかを論ぜよ。

10. <span id="exq-ch5-010"></span>近似アルゴリズムと複雑性理論の関係を調査し、近似困難性結果（例：PCP定理）について説明せよ。

11. <span id="exq-ch5-011"></span>量子計算複雑性クラス（BQP、QMA等）について調査し、古典的な複雑性クラスとの関係を論ぜよ。

12. <span id="exq-ch5-012"></span>実用的な暗号システムが依拠する困難性仮定を調査し、複雑性理論との関連を説明せよ。

### 実装課題

13. <span id="exq-ch5-013"></span>NP完全問題のソルバーを実装せよ：
    ```python
    class SATSolver:
...
14. <span id="exq-ch5-014"></span>計算複雑性の実験的解析を実装せよ：
    ```python
...
15. <span id="exq-ch5-015"></span>確率的アルゴリズムのシミュレータを実装せよ：
    - Miller-Rabin 素数判定法
    - 誤り確率の増幅実験
