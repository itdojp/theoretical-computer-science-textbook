...
**変数ガジェットとの接続**：
各節リテラル頂点を、同じリテラルを表す変数ガジェット側の頂点と辺で結ぶ。割当ての解釈は「変数ガジェットで選ばなかったリテラルを真」とする。

**パラメータ**：
\\(k = n + 2m\\)

**正当性の要点**：
- 充足割当てがあるなら、各変数ガジェットでは偽になるリテラル側を選び、各節ガジェットでは真になるリテラルを1つ未選択にして残り2頂点を選ぶ。接続辺も含めてすべての辺が覆われ、合計は n + 2m 個。
- サイズ k 以下の頂点被覆があるなら、各変数ガジェットから1頂点、各節三角形から2頂点を選ぶ必要がある。各節で未選択のリテラル頂点は、接続辺を覆うために変数ガジェット側の同じリテラル頂点が選ばれていないことを強制し、そのリテラルを真と読める。したがって各節に少なくとも1つ真リテラルがある。
