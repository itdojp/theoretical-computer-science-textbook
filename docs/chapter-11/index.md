...
ここで \\(\\mathrm{negl}(n)\\) は無視できる関数（任意の多項式より速く 0 に収束）。この記述は計算論的安全性の直観であり、実際の安全性 notion では、IND-CPA、IND-CCA、EUF-CMA などの攻撃者能力と成功条件をゲームとして固定し、その優位性がセキュリティパラメータに対して無視できることを要求する。

### 11.1.3 一方向関数

**定義 11.4** 関数 \\(f: \\{0,1\\}^{\ast} \to \\{0,1\\}^{\ast}\\) が**一方向関数**であるとは：
{: #def-11-4 }
1. f は多項式時間で計算可能
2. すべての確率的多項式時間アルゴリズム A に対して：
   \\(\\Pr[f(A(f(x))) = f(x)] < \\mathrm{negl}(n)\\)
   （x は一様ランダムに選択）
