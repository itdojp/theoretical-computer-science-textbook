...
{: #def-4-1 }
HALT_TM = {⟨M, w⟩ \\(\mid\\) M はチューリング機械で、入力 w で停止する}
...
{: #def-4-3 }
A_TM = {⟨M, w⟩ \\(\mid\\) M はチューリング機械で、w を受理する}
...
**定理 4.5** E_TM = {⟨M⟩ \\(\mid\\) L(M) = ∅} は決定不能である。
{: #thm-4-5 }
...
**定理 4.6** EQ_TM = {⟨M₁, M₂⟩ \\(\mid\\) L(M₁) = L(M₂)} は決定不能である。
{: #thm-4-6 }
...
{: #def-4-6 }
- Σ₀ = Π₀ = 決定可能言語
- Σₙ₊₁ = {L \\(\mid\\) L は Πₙ 言語へのオラクルで認識可能}
- Πₙ₊₁ = {L \\(\mid\\) \\(\overline{L}\\) ∈ Σₙ₊₁}
...
**定理 4.9**（Riceの定理）L_P = {⟨M⟩ \\(\mid\\) L(M) は性質 P を持つ} とする。
{: #thm-4-9 }
...
2. {⟨M⟩ \\(\mid\\) L(M) は正規言語}
3. {⟨M⟩ \\(\mid\\) L(M) は文脈自由言語}
4. {⟨M⟩ \\(\mid\\) \\(\lvert L(M)\rvert\\) = k}（k は定数）
5. {⟨M⟩ \\(\mid\\) L(M) は決定可能}
...
- 6. {⟨M⟩ \\(\mid\\) L(M) は無限}（有限性の補性質。非自明なため同様に決定不能）
- 7. {⟨M⟩ \\(\mid\\) 固定語 w₀ が L(M) に含まれる}（特定語の包含も言語の性質であり、非自明なら決定不能）
- 8. {⟨M⟩ \\(\mid\\) L(M) は補有限（cofinite）}（ある有限集合を除き Σ* を含む性質。非自明）
...
1. REGULAR_TM = {⟨M⟩ \\(\mid\\) L(M) は正規言語} が決定不能であることを、Rice の定理を用いて示せ。
2. CONTAINS_{w₀} = {⟨M⟩ \\(\mid\\) 固定語 w₀ が L(M) に含まれる} が決定不能であることを示せ。
...
**定義 4.10** 
{: #def-4-10 }
- P^A = {L \\(\mid\\) ある多項式時間オラクル機械 M^A が L を決定}
- NP^A = {L \\(\mid\\) ある非決定性多項式時間オラクル機械 M^A が L を認識}
...
1. 以下の言語が決定可能か、認識可能か、またはどちらでもないか判定し、証明せよ：
   (a) {⟨M⟩ \\(\mid\\) M は偶数長の文字列のみを受理する}
   (b) {⟨M⟩ \\(\mid\\) M は少なくとも10個の文字列を受理する}
   (c) {⟨M, w, k⟩ \\(\mid\\) M は w を k ステップ以内に受理する}

2. A_TM から以下の言語への多対一還元を構成せよ：
   (a) REGULAR_TM = {⟨M⟩ \\(\mid\\) L(M) は正規言語}
   (b) ALL_TM = {⟨M⟩ \\(\mid\\) L(M) = \\(\Sigma^*\\)}
...
6. K = {x \\(\mid\\) φₓ(x) は定義される} とする。K が創造的集合（creative set）であることを示せ。
