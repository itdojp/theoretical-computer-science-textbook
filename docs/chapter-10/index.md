...
**定義 10.2** 有限または可算な値域を持つ離散確率変数 X の**エントロピー**：
{: #def-10-2 }
\\(H(X) = -\sum_{x} p(x) \log_2 p(x) = \mathbb{E}[I(X)]\\)

ここで、\\(0\log_2 0 = 0\\) と定義する（連続性による）。可算値域では和が発散し、エントロピーが \\(+\infty\\) となる場合がある。
...
和は \\(p(x,y)>0\\) の組について取る。離散分布では、\\(p(x)p(y)=0\\) なら必ず \\(p(x,y)=0\\) なので、そのような組は和に寄与しない。連続型の場合は密度と積分による別定義を用いる。
...
ここでは P が Q に対して絶対連続である、すなわち \\(p(x)>0\\) なら \\(q(x)>0\\) と仮定する。\\(p(x)>0, q(x)=0\\) の項がある場合は \\(D(P \,\Vert\, Q)=\infty\\) と定義する。

**定理 10.4**（情報不等式）\\(D(P \,\Vert\, Q) \ge 0\\)、等号は P と Q が同じ分布のとき。
{: #thm-10-4 }
...
**定理 10.10** Huffman 符号は、記号確率が既知で各記号を二元の整数長接頭語符号に割り当てる条件のもとで、平均符号長を最小化する瞬時符号である。
{: #thm-10-10 }
...
{: #def-10-8 }
有限入力アルファベット \\(\mathcal{X}\\)、有限出力アルファベット \\(\mathcal{Y}\\)、
遷移確率 \\(p(y \mid x)\\) で特徴付けられる。無記憶性は、長さ n の入力列に対して遷移確率が \\(\prod_{i=1}^{n} p(y_i \mid x_i)\\) と積に分解することを意味する。
...
最大化は入力分布 \\(p(x)\\) 全体にわたって行い、対数の底を 2 とするため単位は bits/channel use である。
...
以下では、有限アルファベット上の離散無記憶通信路を固定し、各メッセージを一様に選ぶときの**平均誤り率**を用いる。ブロック長 \\(n\\)、メッセージ数 \\(M_n\\) の符号 \\(C_n\\) について、送信メッセージを \\(J\\)、復号結果を \\(\\widehat J\\) とすると、
...
**定義 10.11** 符号列 \\(\\mathcal{C}=\\{C_n\\}\\) の**漸近レート**と、その符号列に固有の**平均誤り指数**をそれぞれ
{: #def-10-11 }
ここで固定した符号列のレートと平均誤り指数を次式で分離して定める。

\\[
R(\\mathcal{C})
= \\liminf_{n\\to\\infty} \\frac{1}{n}\\log_2 M_n,
\\qquad
E_{\\mathrm{avg}}(\\mathcal{C})
= \\liminf_{n\\to\\infty}
  \\left(-\\frac{1}{n}\\log_2 P_{e,\\mathrm{avg}}^{(n)}\\right)
\\]

と定義する。\\(P_{e,\\mathrm{avg}}^{(n)}=0\\) の項では \\(-\\log_2 0=+\\infty\\) と約束する。下極限を使うことで、通常の極限が存在しない場合にも、符号列が最終的に保証する指数的減少率を表せる。

非負の実数 \\(E\\) がレート \\(R\\) で**達成可能な誤り指数**であるとは、\\(R(\\mathcal{C}) \\ge R\\) かつ \\(E_{\\mathrm{avg}}(\\mathcal{C}) \\ge E\\) を満たす符号列 \\(\\mathcal{C}\\) が存在することをいう。レート \\(R\\) の**信頼性関数**を、達成可能な指数の上限

\\[
E_{\\mathrm{rel}}(R)
= \\sup\\{E \\ge 0 \\mid E \\text{ はレート } R \\text{ で達成可能}\\}
\\]

と定義する。固定した符号列の量 \\(E_{\\mathrm{avg}}(\\mathcal{C})\\) と、符号列全体を最適化する \\(E_{\\mathrm{rel}}(R)\\) は別の量である。

**定理 10.14** 容量 \\(C\\) の離散無記憶通信路と \\(0<R<C\\) に対し、ある \\(E>0\\) と符号列 \\(\\mathcal{C}=\\{C_n\\}\\) が存在して、\\(R(\\mathcal{C})\\ge R\\) であり、十分大きなすべての \\(n\\) について
{: #thm-10-14 }
すなわち、容量未満の各正レートでは、次の指数的誤り上界を満たす符号列を選べる。

\\[
P_{e,\\mathrm{avg}}^{(n)} \\le 2^{-nE}
\\]

となる。したがって \\(E_{\\mathrm{rel}}(R)>0\\) である。

定理10.13と同じ平均誤り基準を用いているが、定理10.14は適切な符号列を選べば誤り率を指数的に減少させられる、という速度まで述べる存在定理である。任意の符号列について正の指数を主張しているのではない。例えば、復号器が通信路出力を無視して常に同じメッセージを出力する符号列では \\(P_{e,\\mathrm{avg}}^{(n)}=1-1/M_n\\) であり、メッセージ数が指数的に増えても \\(E_{\\mathrm{avg}}(\\mathcal{C})=0\\) となる。

## 10.4 誤り訂正符号

### 10.4.1 線形符号

**定義 10.12** **線形符号** \\(C\\) は有限体 \\(\mathbb{F}_q\\)（q は素数冪）上の \\(\mathbb{F}_q^n\\) の線形部分空間。
{: #def-10-12 }
- n：符号長
...
**生成行列と検査行列**：
- 生成行列 \\(G\\)（\\(k \times n\\)）：\\(C = \\{uG \mid u \in \mathbb{F}_q^k\\}\\)
- 検査行列 \\(H\\)（\\((n-k) \times n\\)）：\\(C = \\{c \in \mathbb{F}_q^n \mid Hc^T = 0\\}\\)
- \\(GH^T = 0\\)
...
#### Reed-Solomon 符号
- パラメータ：[n, k, n-k+1] over \\(\mathbb{F}_q\\)（標準的な Reed-Solomon 符号では \\(n \\le q\\)）
- MDS 符号
- バースト誤り訂正に優れる
...
**注意**：離散エントロピーと異なり、h(X) は負になりうる。また、微分エントロピーは変数変換や単位の取り方に依存するため、離散エントロピーと同じ「絶対的な情報量」としては扱わない。
...
{: #def-10-15 }
\\(Y = X + Z\\)、\\(Z \\sim \\mathcal{N}(0, N)\\)。ここで N は1チャネル使用あたりの雑音分散を表す。
...
**定理 10.19** 有限メッセージ空間 \\(\mathcal{M}\\) 上で正しく復号できる暗号方式が完全秘匿性を満たすには、鍵空間 \\(\mathcal{K}\\) について \\(\lvert \mathcal{K}\rvert \ge \lvert \mathcal{M}\rvert\\) が必要。
{: #thm-10-19 }
...
1. <span id="exq-ch10-001"></span>以下の確率分布のエントロピーを計算せよ：
   \\((a) P(X) = \\{1/2, 1/4, 1/8, 1/8\\}\\)
   (b) 幾何分布：\\(P(X=k)=(1-p)^{k-1}p,\\ k=1,2,\\dots\\)

- **詳細解答**: [付録Cの対応解答](../appendices/c/#ex-sol-ch10-001)

2. <span id="exq-ch10-002"></span>X, Y, Z が Markov 連鎖 \\(X \\to Y \\to Z\\) をなすとき、
   \\(I(X;Y \mid Z) + I(X;Z) = I(X;Y)\\) を証明せよ。

- **詳細解答**: [付録Cの対応解答](../appendices/c/#ex-sol-ch10-002)

3. <span id="exq-ch10-003"></span>二元対称通信路（クロスオーバー確率 0.1）において、
   入力が一様分布のときの相互情報量を計算せよ。

- **詳細解答**: [付録Cの対応解答](../appendices/c/#ex-sol-ch10-003)

4. <span id="exq-ch10-004"></span>{a:0.5, b:0.25, c:0.125, d:0.125} に対する Huffman 符号を構成し、
   平均符号長を求めよ。

- **詳細解答**: [付録Cの対応解答](../appendices/c/#ex-sol-ch10-004)

### 発展問題

5. <span id="exq-ch10-005"></span>Fano の不等式を証明し、通信路符号化定理の逆定理への応用を説明せよ。

- **詳細解答**: [付録Cの対応解答](../appendices/c/#ex-sol-ch10-005)

6. <span id="exq-ch10-006"></span>型（typical sequences）の理論について：
   (a) 弱型と強型の定義を与えよ
   (b) 漸近等分割性（AEP）を証明せよ

- **詳細解答**: [付録Cの対応解答](../appendices/c/#ex-sol-ch10-006)

7. <span id="exq-ch10-007"></span>Slepian-Wolf の定理（分散情報源符号化）を説明し、
   達成可能領域を特徴付けよ。

- **詳細解答**: [付録Cの対応解答](../appendices/c/#ex-sol-ch10-007)

8. <span id="exq-ch10-008"></span>ネットワーク符号化について：
   (a) 最大流最小カット定理との関係を説明せよ
   (b) 線形ネットワーク符号の十分性を論ぜよ

- **詳細解答**: [付録Cの対応解答](../appendices/c/#ex-sol-ch10-008)

### 探究課題

9. <span id="exq-ch10-009"></span>量子情報理論について調査し、
   von Neumann エントロピーと古典的エントロピーの違いを論ぜよ。

- **調査ガイド**: [付録Cの対応解答](../appendices/c/#ex-sol-ch10-009)

10. <span id="exq-ch10-010"></span>Kolmogorov 複雑性について調査し、
    Shannon エントロピーとの関係を説明せよ。

- **調査ガイド**: [付録Cの対応解答](../appendices/c/#ex-sol-ch10-010)

11. <span id="exq-ch10-011"></span>極符号（Polar codes）について調査し、
    通信路分極現象と容量達成性を説明せよ。

- **調査ガイド**: [付録Cの対応解答](../appendices/c/#ex-sol-ch10-011)

12. <span id="exq-ch10-012"></span>深層学習における情報理論的解析について調査し、
    情報ボトルネック理論の応用例を示せ。

---

- **調査ガイド**: [付録Cの対応解答](../appendices/c/#ex-sol-ch10-012)
