...
補足（既知の障壁の概略）：
- 相対化（relativization）障壁：多くの古典的手法はオラクル相対化するが、相対化世界では P^A = NP^A と P^B ≠ NP^B が両立し得るため、相対化する証明法では P≠NP を示せない可能性がある。
- 自然な証明（natural proofs）障壁：布施–Razborov–Rudich により、擬似乱数生成器の存在を仮定すると「大規模」「構造的」の性質に基づく“自然な”下界手法は強力すぎて破れないことが示唆される。
- 代数化（algebraization）障壁：相対化を拡張した代数化の枠組みでも、P vs NP に本質的な進展を阻むことが示唆される。これらは「どの種の手法が効かないか」を示すメタ結果である。
