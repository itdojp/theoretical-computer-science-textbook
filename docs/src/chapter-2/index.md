...
- **アルゴリズムの限界**：「計算不可能」問題の厳密な定義
- **計算複雑性理論**：P、NP等の複雑性クラスの基礎
- **プログラミング言語設計**：計算完全性（Turing completeness）の概念
- **人工知能**：機械による知能の理論的限界の探求
...
**ステップ1: 仮定の設定**
停止問題を決定するチューリング機械 H が存在すると仮定します。H は以下のように動作します：
...
**ステップ3: 自己適用によるパラドックス**
D を自分自身に適用します：D(⟨D⟩) はどうなるでしょうか？

- **Case 1**: D が ⟨D⟩ で停止すると仮定
  - H(⟨D, ⟨D⟩⟩) = 受理
  - D の定義により、D(⟨D⟩) は無限ループ
  - **矛盾**：D が停止すると仮定したが、無限ループになる

- **Case 2**: D が ⟨D⟩ で停止しないと仮定
  - H(⟨D, ⟨D⟩⟩) = 拒否  
  - D の定義により、D(⟨D⟩) は停止
  - **矛盾**：D が停止しないと仮定したが、停止する
