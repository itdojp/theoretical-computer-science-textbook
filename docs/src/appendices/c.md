...
### 練習問題6.x 置換法：T(n) = 3T(n/2) + n を解け

【解答】
- 期待される解: T(n) = Θ(n^{log₂3})（≃ Θ(n^{1.585})）。
- 置換法の一例: 帰納法で T(n) ≤ c·n^{log₂3} − k·n を仮定し、
  T(n) = 3((c(n/2)^{log₂3} − k·(n/2)) + n ≤ 3c·n^{log₂3}/2^{log₂3} − (3k/2)n + n
  = 3c·n^{log₂3}/3 − (3k/2 − 1)n = c·n^{log₂3} − (3k/2 − 1)n
  k を十分大きく取れば帰納が閉じる。
 下界も同様に示す（またはマスター定理の条件から到達）。□

### 補足: 再帰式バリエーションの目安

1) T(n) = 4T(n/2) + n^2 → a=4, b=2, f(n)=n^2、n^{log_b a} = n^2 ⇒ ケース2（同次数）で T(n)=Θ(n^2 log n)。

2) T(n) = T(n/2) + n / log n → a=1, b=2, f(n)=n/log n、n^{log_b a}=n^0=1。拡張版のマスター定理や置換法で T(n)=Θ(n) を示せる（調和級数的寄与が主任項）。
...
【理由】右同値関係 ≡ₗ で、接尾辞の付加により 5 つの剰余類（mod 5）の右コンテキストが相互に区別可能。
よって同値類の個数 ≥ 5 ⇒ 最小DFAの状態数 ≥ 5（実際に 5 状態で実現可能）。□

### 補足: 最小DFAの下界（L₄: mod 7）

【主張】L₄ = { w ∈ {0,1}* | val₂(w) ≡ 0 (mod 7) } の最小DFAは少なくとも 7 状態。

【理由】mod 7 の 7 つの剰余類が、それぞれ右コンテキストとして相互に識別可能になる。よって同値類 ≥ 7 ⇒ 下界 7（実際に 7 状態のDFAで実現可能）。□
