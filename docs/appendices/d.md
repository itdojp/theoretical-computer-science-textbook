...
**無視できる関数（Negligible Function）**
- 任意の正の多項式 \(p\) に対し、十分大きな \(n\) で \(\mu(n)<1/p(n)\) となる非負関数
- 暗号の安全性ゲームでは、すべてのPPT攻撃者の優位性をこの関数で上から抑える（→ [第11章: 暗号理論の数学的基礎](../chapter-11/index.md)）
...
- 一様入力の像から、同じ像を持つ任意の原像をPPT攻撃者が求める成功確率が無視できる関数族
- 順方向は決定的多項式時間で計算できるが、逆向きの困難性は入力分布と計算困難性仮定に依存する
...
- 正の確率を持つ平文と暗号文について、暗号文を観測しても平文の事後分布が事前分布から変化しない性質
- 鍵・メッセージ・暗号化乱数の分布と独立性を定めた確率実験として定義する（→ [第11章: 暗号理論の数学的基礎](../chapter-11/index.md)）
...
- 短い一様乱数種から、より長く、PPT識別器には一様乱数と見分けにくい列を生成する決定的多項式時間関数
- 出力長は入力長より大きく、多項式で抑えられる伸長長として定める
