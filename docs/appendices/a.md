...
- 補集合：`\\(\overline{L}\\)`（`L̄` 等の合成文字の直書きは避ける）
- Kleene 星：`\\(\Sigma^*\\)`、`\\(\\{0,1\\}^*\\)` のように数式中で表記する
...
| \ | 差集合 | 集合の差 | A \ B |
| × | 直積 | デカルト積 | A × B |
| P(A) | べき集合 | Aのすべての部分集合 | P({1,2}) = {∅,{1},{2},{1,2}} |
...
- Markdownの表では `|` が区切り文字になるため、集合の濃度・長さ・内包表記の区切りは `|...|` や `{x | ...}` を直書きせず、`\\(\lvert\cdot\rvert\\)` と `\mid` を用いて数式として記述します。
- Kleene 星（`*`）は Markdown の強調と衝突しうるため、`\(\Sigma^*\)` のように数式中で表記し、正規表現はインラインコードに統一します。
