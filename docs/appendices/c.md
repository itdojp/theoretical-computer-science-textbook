...
**問題**: 言語 \\(L = \\{\\,w \\in \\{a,b\\}^* \\mid \\text{w に a が偶数個含まれる}\\,\\}\\) が正規言語であることを証明せよ。
...
**問題**: 章末問題「発展 8.」について、以下を示せ。
 (a) \\(L_1 = \\{\\,w \\in \\{a,b\\}^* \\mid \\text{w が部分文字列 aba を含む}\\,\\}\\) の最小DFAは少なくとも4状態を要する。
 (b) \\(L_2 = \\{\\,w \\in \\{0,1\\}^* \\mid \\text{w 中の 1 の個数が 0 (mod 3) で、最終文字が 0}\\,\\}\\) の最小DFAは少なくとも6状態を要する。
...
【主張】\\(L_3 = \\{\\,w \\in \\{0,1\\}^* \\mid \\mathrm{val}_2(w) \\equiv 0 \\pmod{5}\\,\\}\\) の最小DFAは少なくとも 5 状態。
...
【主張】\\(L_4 = \\{\\,w \\in \\{0,1\\}^* \\mid \\mathrm{val}_2(w) \\equiv 0 \\pmod{7}\\,\\}\\) の最小DFAは少なくとも 7 状態。
...
非自明（例: ∅ は有限、\\(\Sigma^*\\) は非有限）。よって Rice の定理により \\(\\{\\,\\langle M\\rangle \\mid L(M) \\text{ が } P \\text{ を満たす}\\,\\}\\) は決定不可能。□

### 練習問題4.6 COFINITE_TM の非決定可能性（Rice）

**問題**: \\(\\mathrm{COFINITE}_{\\mathrm{TM}} = \\{\\,\\langle M\\rangle \\mid \\Sigma^* \\setminus L(M) \\text{ が有限（補有限）}\\,\\}\\) が決定可能か判定し、理由を述べよ。

**解答**: 決定不可能。

**証明**: 性質 P を「補集合が有限（補有限）」とする。P は言語の意味的性質であり、
非自明（\\(\Sigma^*\\) は満たす、∅ は満たさない）。従って Rice の定理により \\(\\{\\,\\langle M\\rangle \\mid L(M) \\text{ が } P \\text{ を満たす}\\,\\}\\) は決定不可能。□
