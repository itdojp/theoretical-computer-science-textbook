...
\\(\\mathrm{ATM} = \\{\\,\\langle M, w\\rangle \\mid M \\text{ は入力 } w \\text{ で停止し受理する}\\,\\}\\)
...
**Riceの定理（要約）**: 言語 L(M) にのみ依存する任意の非自明な性質 P について、集合 \\(\\{\\,\\langle M\\rangle \\mid L(M) \\text{ が } P \\text{ を満たす}\\,\\}\\) は決定不可能（→ 第4章: ../chapter-4/index.md）。
...
（問題要約）4.2.3 のテンプレに従い、\\(ATM \\le_m \\overline{E_{TM}}\\) または \\(ATM \\le_m ALL_{TM}\\) の構成を概説せよ。

【解答の骨子】
- 目的: \\(A \\le_m B\\)。\\(A = ATM\\)、\\(B = \\overline{E_{TM}}\\) または \\(B = ALL_{TM}\\)。
