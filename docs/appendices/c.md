...
`choosing[j]` は、`j` が1行目を実行した後、2行目の `number[j]` が現在の要求の番号へ確定する前に、`i` が中間状態の `number[j] = 0` を読んで待機を通過することを防ぐ。`i` は `choosing[j] = false` を確認してから、確定して6行目まで安定している `number[j]` を比較する。この順序がなければ、番号選択中の相手を「要求なし」と誤認でき、上の不変条件を導けない。
