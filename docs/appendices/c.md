...
**問題**: 固定の語 w₀ に対して \\(\\mathrm{CONTAINS}_{w_0} = \\{\\,\\langle M\\rangle \\mid w_0 \\in L(M)\\,\\}\\) が決定可能か判定し、理由を述べよ。
...
（問題要約）4.2.3 のテンプレに従い、ATM ≤ₘ \\(\overline{E_{TM}}\\) または ATM ≤ₘ ALL_TM の構成を概説せよ。

【解答の骨子】
- 目的: A ≤ₘ B。A = ATM、B = \\(\overline{E_{TM}}\\) または ALL_TM。
...
**NP困難性の証明**（3-SATからの還元）:
3-SATのインスタンス φ = C₁ ∧ C₂ ∧ ... ∧ Cₘ（各Cᵢは3つのリテラルの選言）から、グラフG = (V,E)とk = \\(\lvert V\rvert\\) − mを構成：

**構成**:
1. 各変数 xᵢ について：2つの頂点 \\(xᵢ\\), \\(\lnot xᵢ\\) を作成し、エッジ \\((xᵢ, \lnot xᵢ)\\) を追加
...
構成の復習：
- 各変数につき 2 頂点（\\(xᵢ\\), \\(\lnot xᵢ\\)）と 1 辺（相互排他エッジ）→ 頂点 2n、辺 n
...
以上より、(G,k) を (Ḡ, \\(\lvert V\rvert\\) − k) に写す写像は CLIQUE ≤ₚ VERTEX-COVER の多項式時間還元を与える。
