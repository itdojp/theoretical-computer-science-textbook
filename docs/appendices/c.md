...
**問題**: L = {w ∈ {a,b}* \\(\mid\\) wにaが偶数個含まれる}が正規言語であることを証明せよ。
...
**問題**: L = {aⁿbⁿ \\(\mid\\) n ≥ 0}を生成する文脈自由文法を示せ。
...
**問題**: L = {aⁿbⁿcⁿ \\(\mid\\) n ≥ 0}が文脈自由言語でないことをポンピング補題で証明せよ。
...
**問題**: 章末問題「発展 8.」について、以下を示せ。
 (a) L₁ = { w ∈ {a,b}* \\(\mid\\) w が部分文字列 "aba" を含む } の最小DFAは少なくとも4状態を要する。
 (b) L₂ = { w ∈ {0,1}* \\(\mid\\) #1(w) ≡ 0 (mod 3) かつ最終文字が 0 } の最小DFAは少なくとも6状態を要する。
...
【主張】L₃ = { w ∈ {0,1}* \\(\mid\\) val₂(w) ≡ 0 (mod 5) } の最小DFAは少なくとも 5 状態。
...
【主張】L₄ = { w ∈ {0,1}* \\(\mid\\) val₂(w) ≡ 0 (mod 7) } の最小DFAは少なくとも 7 状態。
...
**問題**: L = { 0ⁿ1ⁿ \\(\mid\\) n ≥ 0 } が正規言語でないことを Myhill–Nerode の定理で示せ。
...
EMPTY = {⟨M⟩ \\(\mid\\) M はチューリング機械でL(M) = ∅}

**解答**:

**証明**: 停止問題ATMからEMPTYへの還元を示す。

ATM = {⟨M,w⟩ \\(\mid\\) M は入力wで停止し受理する}
...
INFINITE = {⟨M⟩ \\(\mid\\) L(M)は無限言語}

**解答**:

**証明**: Riceの定理を適用する。

**Riceの定理（要約）**: 言語 L(M) にのみ依存する任意の非自明な性質 P について、集合 {⟨M⟩ \\(\mid\\) L(M) が P を満たす} は決定不可能（→ 第4章: ../chapter-4/index.md）。
...
**問題**: REGULAR_TM = {⟨M⟩ \\(\mid\\) L(M) は正規言語} が決定可能か判定し、理由を述べよ。

**解答**: 決定不可能。

**証明**: Rice の定理を適用する。性質 P を「正規言語である」とする。
P は L(M)（言語）にのみ依存する意味的性質であり、非自明（∅ や `a*` は正規、{a^n b^n \\(\mid\\) n≥0} は非正規）である。したがって
{⟨M⟩ \\(\mid\\) L(M) が P} は決定不可能。□

### 練習問題4.4 固定語包含性 CONTAINS_{w₀} の非決定可能性（Rice）

**問題**: 固定の語 w₀ に対して CONTAINS_{w₀} = {⟨M⟩ \\(\mid\\) w₀ ∈ L(M)} が決定可能か判定し、理由を述べよ。

**解答**: 決定不可能。

**証明**: 性質 P を「w₀ を含む」とする。
P は言語の意味的性質であり、非自明（Σ* は満たす、∅ は満たさない）。よって
{⟨M⟩ \\(\mid\\) L(M) が P} は Rice の定理により決定不可能。□
...
**問題**: FINITE_TM = {⟨M⟩ \\(\mid\\) L(M) は有限言語} が決定可能か判定し、理由を述べよ。

**解答**: 決定不可能。

**証明**: 性質 P を「言語が有限である」とする。P は L(M) にのみ依存する意味的性質で、
非自明（例: ∅ は有限、\\(\Sigma^*\\) は非有限）。よって Rice の定理により {⟨M⟩ \\(\mid\\) L(M) が P} は決定不可能。□

### 練習問題4.6 COFINITE_TM の非決定可能性（Rice）

**問題**: COFINITE_TM = {⟨M⟩ \\(\mid\\) \\(\Sigma^* \setminus L(M)\\) が有限（補有限）} が決定可能か判定し、理由を述べよ。

**解答**: 決定不可能。

**証明**: 性質 P を「補集合が有限（補有限）」とする。P は言語の意味的性質であり、
非自明（\\(\Sigma^*\\) は満たす、∅ は満たさない）。従って Rice の定理により {⟨M⟩ \\(\mid\\) L(M) が P} は決定不可能。□
...
VERTEX-COVER = {⟨G,k⟩ \\(\mid\\) Gはサイズk以下の頂点被覆を持つ}
...
- 補グラフの構成は O(\\(\lvert V\rvert\\)²) 時間で可能（全ての頂点対を走査して辺の有無を反転）
