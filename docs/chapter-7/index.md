...
**定理 7.2**（比較ベース辞書の下界）n 個の異なる要素を保持する比較ベースの辞書（ordered dictionary / decision tree model）において、
{: #thm-7-2 }
membership query（要素の存在確認）は最悪の場合 \\(\\Omega(\\log n)\\) 回の比較を要する。
...
注: 定理 7.2 の下界は、比較で分岐する探索モデルに対する主張である。
ハッシュ表はこの下界の対象外であり、以下の \\(O(1)\\) は単純一様ハッシュの仮定の下での期待時間を述べている。
