...
- **CIの概要**: GitHub Actionsでビルドと静的チェックを実施します。
  - ビルド: `jekyll build`（本番相当）
  - Lint: `markdownlint`（Markdownスタイル）
  - リンクチェック: `markdown-link-check`（内部リンクは厳密、外部はベストエフォート）
  - スペルチェック: 今後 `cspell` の導入を予定（現状はプレースホルダー）

- **ローカルでの実行**:
  - `npm run build` — 本番ビルド
  - `npm run lint` — Markdown lint
  - `npm run check-links` — リンクチェック（ネットワーク状況により外部は失敗する場合があります）
  - `npm run spellcheck` — 近く `cspell` に切替予定（導入PR歓迎）

- **対象/除外**:
  - 主対象: `docs/src/**/*.md`
  - 除外: 生成物（`docs/_site/**`）、バイナリ/画像（`docs/assets/**`）、`node_modules/**` ほか

- **PRの受入れ基準（目安）**:
  - ビルド成功（`npm run build`）
  - Lint・リンクチェックで重大な警告/エラーなし（外部リンクはタイムアウト時に再試行/許容）
  - スタイルガイド準拠（log₂・内部リンクの方針・実務メモの位置付け）

- **リンクの扱い**:
  - 内部リンクは末尾スラッシュ/拡張子なしのPretty URLで統一
  - 外部リンクは可能ならHTTPS・安定なURLに。壊れたリンクはIssue化 or 置換

- **将来の拡張**:
  - `cspell` の導入と辞書の整備（`cspell.json` あり）
  - 数学表記/図表に対する追加ルール（例: 記号の統一、SVGアクセシビリティ属性）
