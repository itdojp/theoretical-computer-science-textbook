...
以下の定理は、直前の線形探査ではなく、各キーの探査列が一様ランダムに振る舞うとみなす理想化モデル（uniform hashing）について述べる。
...
線形探査はこの uniform hashing モデルそのものではなく、一次クラスタリングにより期待探査数がこの理想化より悪化し得る。実装では負荷率の上限と再ハッシュ戦略が重要になる。
