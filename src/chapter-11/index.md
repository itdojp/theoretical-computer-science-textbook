...
- \\(E = \\{E_k : \\mathcal{P} \\to \\mathcal{C} \\mid k \\in \\mathcal{K}\\}\\)：暗号化関数の族
- \\(D = \\{D_k : \\mathcal{C} \\to \\mathcal{P} \\mid k \\in \\mathcal{K}\\}\\)：復号関数の族

決定的暗号化の正当性条件は、すべての \\(k \\in \\mathcal{K}\\) と \\(m \\in \\mathcal{P}\\) について \\(D_k(E_k(m))=m\\) となることである。ランダム化暗号では独立な暗号化乱数 \\(\\rho\\) を明示して \\(E_k(m;\\rho)\\) と書き、乱数空間のsupportに属するすべての \\(\\rho\\) について \\(D_k(E_k(m;\\rho))=m\\) を要求する。決定的暗号化は乱数空間が1要素の場合とみなせる。
...
を満たすことをいう。確率はゲームの挑戦者と攻撃者 \\(A\\) の内部乱択の両方に関する。関数 \\(\\mu\\) が**無視できる**とは、任意の正の多項式 \\(p\\) に対して、十分大きなすべての \\(n\\) で \\(\\mu(n)<1/p(n)\\) となることをいう。
...
2. すべてのPPT攻撃者 \\(A\\) に対し、ある無視できる関数 \\(\\mu_A\\) が存在する。\\(x\\leftarrow\\{0,1\\}^n\\) を一様に選び、\\(A\\) の内部乱数 \\(\\rho_A\\) を独立に選んで \\(x^{\\prime}=A(1^n,f_n(x);\\rho_A)\\) とすると、

   \\[
   \\Pr[f_n(x^{\\prime})=f_n(x)] \\le \\mu_A(n)
   \\]
