...
ここで \\(\\mathrm{negl}(n)\\) は無視できる関数（任意の多項式より速く 0 に収束）。
...
NIST はさらに、ML-KEM のバックアップ KEM として **HQC** を標準化対象に選定しており、FALCON 由来の署名方式（FN-DSA/FIPS 206）も追加標準として扱われる予定である。これらの最終文書化状況は変わり得るため、実運用では NIST の最新文書を確認する。
