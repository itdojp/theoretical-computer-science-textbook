...
この定理が保証するのは、自身の記述に依存する振る舞いを持つ機械の存在である。任意のプログラムが自動的に自身のソースコードを読み込んだり、変更したりできることを直接保証するものではない。なお、ここでの「受理」は出力された機械記述が表す機械の振る舞いを指す。 なお、ここでの「受理」は出力された機械記述が表す機械の振る舞いを指す。
...
8. チューリング機械の停止性を判定する「近似アルゴリズム」を考える。分布、誤判定の許容、未回答（unknown）の許容を明確にしないと「一定割合以上で正しい」という主張が数学的命題にならないことを説明せよ。そのうえで、すべての入力に対して必ず停止し必ず正しく答える完全判定器は存在しないことを、停止問題の証明に戻って整理せよ。
