...
**例 2.2** 言語 L = {0ⁿ1ⁿ \\(\mid\\) n ≥ 0} を認識するチューリング機械
{: #ex-2-2 }
...
**例 2.4** 
{: #ex-2-4 }
- PRIME = {n \\(\mid\\) n は2進表記された素数}
- CONNECTED = {⟨G⟩ \\(\mid\\) G は連結グラフ}
...
計算量の観点では、t(n) ステップの k-テープ計算は単一テープで概ね O(t(n)²) 時間でシミュレートできる（十分大きな n で t(n) ≥ n を仮定）。
□
...
すると D は Ld = {⟨M⟩ \\(\mid\\) M は ⟨M⟩ を受理しない} を認識することになる。
...
**定理 2.12**（再帰定理）すべてのチューリング機械 T に対して、機械 R が存在し、R と T(⟨R⟩) は同じ言語を認識する。
{: #thm-2-12 }
...
3. 以下の言語が決定可能であることを証明せよ。
   (a) ADFA = {⟨B, w⟩ \\(\mid\\) B は DFA で、B は w を受理する}
   (b) ECFG = {⟨G⟩ \\(\mid\\) G は文脈自由文法で、L(G) = ∅}
   
   ヒント: (b) 到達可能・生成可能な記号の削除で S から終端へ導出が存在するかを判定（標準アルゴリズム）。

4. L₁ = {w \\(\mid\\) w は偶数個の0を含む} と L₂ = {w \\(\mid\\) w は3の倍数個の1を含む} が決定可能であることを示し、L₁ ∩ L₂ を決定するアルゴリズムを示せ。

5. 非決定性チューリング機械が言語 {0ⁿ1ⁿ \\(\mid\\) n ≥ 0} ∪ {0ⁿ1²ⁿ \\(\mid\\) n ≥ 0} を線形時間で認識できることを示せ。
...
例として、{0ⁿ1ⁿ \\(\mid\\) n ≥ 0} を認識する機械を実装し、テストせよ。
