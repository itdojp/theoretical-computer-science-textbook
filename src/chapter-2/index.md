...
**定理 2.12**（再帰定理）プログラム記述を入力として受け取るチューリング機械 T に対して、ある機械 R が存在し、すべての入力 w について \(R\) が w を受理することと、\(T(\langle R\rangle)\) が w を受理することが同値になる。ここで \(\langle R\rangle\) は R の有限な記述、\(T(\langle R\rangle)\) はその記述を入力としてTが出力する機械の記述を表す。
{: #thm-2-12 }

この定理が保証するのは、自身の記述に依存する振る舞いを持つ機械の存在である。任意のプログラムが自動的に自身のソースコードを読み込んだり、変更したりできることを直接保証するものではない。
