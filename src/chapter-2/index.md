...
**定義 2.5** 関数 \\(f: \Sigma^* \to \Sigma^*\\) が**チューリング計算可能**（Turing computable）であるとは、以下を満たすチューリング機械 M が存在することである。
{: #def-2-5 }
- すべての w ∈ Σ* に対して
- M を入力 w で開始すると
...
**定義 2.6** 部分関数 \\(f: \Sigma^* \rightharpoonup \Sigma^*\\) が**チューリング計算可能**であるとは、以下を満たすチューリング機械 M が存在することである。
{: #def-2-6 }
