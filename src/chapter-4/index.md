...
{: #def-4-1 }
HALT_TM = \\(\\{\\langle M,w\\rangle \\mid M \\text{ はチューリング機械で、入力 } w \\text{ で停止する}\\}\\)
...
{: #def-4-3 }
A_TM = \\(\\{\\langle M,w\\rangle \\mid M \\text{ はチューリング機械で、} w \\text{ を受理する}\\}\\)
...
**定理 4.5** E_TM = \\(\\{\\langle M\\rangle \\mid L(M) = \emptyset\\}\\) は決定不能である。
{: #thm-4-5 }

*証明*：A_TM ≤_m \\(\overline{E_{TM}}\\) を示す（\\(\overline{E_{TM}}\\) は E_TM の補集合）。
...
したがって A_TM ≤_m \\(\overline{E_{TM}}\\) が成り立ち、\\(\overline{E_{TM}}\\) は決定不能。
よって E_TM も決定不能である。□

**定理 4.6** EQ_TM = \\(\\{\\langle M_1, M_2\\rangle \\mid L(M_1) = L(M_2)\\}\\) は決定不能である。
{: #thm-4-6 }
...
- 8. \\(\\{\\langle M\\rangle \\mid L(M) \\text{ は補有限（cofinite）}\\}\\)（ある有限集合を除き Σ* を含む性質。非自明）
...
1. REGULAR_TM = \\(\\{\\langle M\\rangle \\mid L(M) \\text{ は正規言語}\\}\\) が決定不能であることを、Rice の定理を用いて示せ。
...
- 決定不能性は対角化や還元により示される（代表例: HALT_TM, A_TM）
...
9. 【テンプレ練習】4.2.3 の「還元の作法テンプレ」に従い、以下のいずれかについて多対一還元の構成を概説せよ：
   (a) \\(ATM \\le_m \\overline{E_{TM}}\\)（EMPTY の補集合）
   \\((b) ATM \\le_m ALL_{TM}\\)
