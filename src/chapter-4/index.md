...
- 還元による決定不能性の証明手順を述べ、基本的な還元を構成できる
...
{: #def-4-1 }
\\(HALT_{TM} = \\{\\langle M,w\\rangle \\mid M \\text{ はチューリング機械で、入力 } w \\text{ で停止する}\\}\\)
...
いずれの場合も矛盾が生じるので、\\(HALT_{TM}\\) は決定不能である。□
...
**定義 4.3** **受理問題** \\(A_{TM}\\) は以下の言語として定義される：
{: #def-4-3 }
\\(A_{TM} = \\{\\langle M,w\\rangle \\mid M \\text{ はチューリング機械で、} w \\text{ を受理する}\\}\\)

**定理 4.3** \\(A_{TM}\\) は認識可能だが決定可能でない。
{: #thm-4-3 }

*証明*：
（認識可能性）万能チューリング機械 U が \\(A_{TM}\\) を認識する。
U は入力 ⟨M, w⟩ に対して M の w での計算をシミュレートし、M が受理すれば U も受理する。

（決定不能性）\\(A_{TM}\\) が決定可能と仮定し、その決定機械を R とする。
以下の機械 D を構成する：
...
{: #def-4-4 }
計算可能関数 \\(f: \\Sigma^{\\ast} \\to \\Sigma^{\\ast}\\) が存在して、すべての \\(w\\) に対して
\\(w \\in A \\iff f(w) \\in B\\)
が成り立つことである。これを \\(A \\le_m B\\) と表記する。

![多対一還元の仕組み]({{ '/assets/images/diagrams/ch4_many_one_reduction.svg' | relative_url }})

**定理 4.4** \\(A \le_m B\\) かつ B が決定可能ならば、A も決定可能である。
{: #thm-4-4 }
...
また、\\(w \\in A \\iff f(w) \\in B\\) より、M_A は A を正しく決定する。□

**系 4.1** \\(A \\le_m B\\) かつ A が決定不能ならば、B も決定不能である。
{: #cor-4-1 }

### 4.2.2 還元による決定不能性の証明

**定理 4.5** \\(E_{TM} = \\{\\langle M\\rangle \\mid L(M) = \\emptyset\\}\\) は決定不能である。
{: #thm-4-5 }
...
もし \\(E_{TM}\\) が決定可能なら補集合 \\(\\overline{E_{TM}}\\) も決定可能になってしまうため、\\(E_{TM}\\) も決定不能である。□

**定理 4.6** \\(EQ_{TM} = \\{\\langle M_1, M_2\\rangle \\mid L(M_1) = L(M_2)\\}\\) は決定不能である。
{: #thm-4-6 }

*証明*：\\(E_{TM} \\le_m EQ_{TM}\\) を示す。
...
このとき：
- \\(L(M) = ∅ ⟺ L(M) = L(M_∅) ⟺ ⟨M, M_∅⟩ ∈ EQ_{TM}\\)

したがって \\(E_{TM} \\le_m EQ_{TM}\\) が成り立ち、\\(EQ_{TM}\\) は決定不能である。□
...
**性質**：
- \\(A \\le_m B\\) ならば \\(A \\le_T B\\)（逆は一般に成立しない）
- \\(\\le_T\\) は推移的：\\(A \\le_T B\\) かつ \\(B \\le_T C\\) ならば \\(A \\le_T C\\)
...
ここで「すべての言語」は、固定した有限アルファベット上の文字列集合 \\(\\Sigma^{\\ast}\\) の部分集合全体を意味する。
...
（真の包含）\\(A_{TM}\\) は再帰的可算だが決定可能でない。

（再帰的可算 ⊂ すべて）対角化言語 \\(L_d\\) はすべての言語の一つだが、定理 4.2 より認識可能でない。
したがって、再帰的可算言語の外側にも言語が存在する。なお \\(\\overline{L_d}\\) は「\\(M\\) が \\(\\langle M\\rangle\\) を受理したら受理する」機械で認識できるため、ここでの非認識可能性の証人は \\(\\overline{L_d}\\) ではなく \\(L_d\\) である。□
...
{: #def-4-6 }
- \\(\\Sigma_0 = \\Pi_0\\)：決定可能言語
- \\(\\Sigma_{n+1} = \\{L \\mid L \\text{ は } \\Pi_n \\text{ 言語へのオラクルで認識可能}\\}\\)
- \\(\\Pi_{n+1} = \\{L \\mid \\overline{L} \\in \\Sigma_{n+1}\\}\\)

**例**：
- \\(\\Sigma_1\\)：再帰的可算言語
- \\(\\Pi_1\\)：補再帰的可算言語（co-RE）
- \\(A_{TM} \\in \\Sigma_1 \\setminus \\Pi_1\\)
- \\(HALT_{TM} \\in \\Sigma_1 \\setminus \\Pi_1\\)
...
1. \\(L \\in \\Sigma_n\\)
2. ある決定可能な関係 R が存在して、
   \\(x \\in L ⟺ \\exists y_1\\forall y_2\\exists y_3\\cdots Q_ny_n\\, R(x, y_1, \\ldots, y_n)\\)
   （\\(Q_n\\) は n が奇数なら \\(\\exists\\)、偶数なら \\(\\forall\\)）
...
{: #def-4-7 }
- ある機械 \\(M_1\\) に対して \\(L(M_1)\\) は性質 \\(P\\) を持つ
- ある機械 \\(M_2\\) に対して \\(L(M_2)\\) は性質 \\(P\\) を持たない
ことである。

### 4.4.2 Riceの定理の主張と証明

**定理 4.9**（Riceの定理）L_P = \\(\\{\\langle M\\rangle \\mid L(M) \\text{ は性質 } P \\text{ を持つ}\\}\\) とする。
{: #thm-4-9 }
...
このとき：
- M が w を受理する場合：\\(L(M^{\\prime}) = L(M_L) = L\\)。L は性質 P を持つので \\(\\langle M^{\\prime}\\rangle \\in L_P\\)
- M が w を受理しない場合：\\(L(M^{\\prime}) = \\emptyset\\)。\\(\\emptyset\\) は性質 P を持たないので \\(\\langle M^{\\prime}\\rangle \\notin L_P\\)
...
1. \\(\\{\\langle M\\rangle \\mid L(M) \\text{ は有限}\\}\\)
2. \\(\\{\\langle M\\rangle \\mid L(M) \\text{ は正規言語}\\}\\)
3. \\(\\{\\langle M\\rangle \\mid L(M) \\text{ は文脈自由言語}\\}\\)
4. \\(\\{\\langle M\\rangle \\mid \\lvert L(M)\\rvert = k\\}\\)（k は定数）
5. \\(\\{\\langle M\\rangle \\mid L(M) \\text{ は決定可能}\\}\\)
...
1. \\(REGULAR_{TM} = \\{\\langle M\\rangle \\mid L(M) \\text{ は正規言語}\\}\\) が決定不能であることを、Rice の定理を用いて示せ。
2. \\(\\mathrm{CONTAINS}_{w_0} = \\{\\langle M\\rangle \\mid w_0 \\in L(M)\\}\\) が決定不能であることを示せ。
...
構成法：
1. **合成**：\\(h(\\vec{x}) = f(g_1(\\vec{x}), \\ldots, g_k(\\vec{x}))\\)
2. **原始再帰**：
   - \\(h(0, \vec{x}) = f(\vec{x})\\)
   - \\(h(n+1, \vec{x}) = g(n, h(n, \vec{x}), \vec{x})\\)
3. **最小化**：\\(h(\vec{x}) = \mu y[f(\vec{x}, y) = 0]\\)
...
{: #thm-4-11 }
ある n が存在して \\(\\varphi_n = \\varphi_{f(n)}\\) となる。

ここで \\(\\varphi_n\\) は符号 n を持つチューリング機械が計算する部分関数。
...
**定義 4.11** 言語 A, B に対して：
{: #def-4-11 }
- \\(A \\equiv_T B\\) は \\(A \\le_T B\\) かつ \\(B \\le_T A\\) が成り立つこと
...
- 決定不能性は対角化や還元により示される（代表例: \\(HALT_{TM}\\), \\(A_{TM}\\)）
- Rice の定理は「言語の性質」に関する広範な非決定可能性を一括で与える
- 実務でも停止性や一般的性質判定の自動化には本質的限界がある

### 次章への橋渡し

第4章では「解けない問題」の境界を見ました。次章では問いを「解けるがどの程度大変か」に切り替え、時間・空間という資源の観点から問題の難しさを比較します。

補助資料を併用するなら、理論の使われ方は [付録E](../appendices/e/) の第4章節、理解確認は [付録F](../appendices/f/) の第4章節を参照してください。
...
- **次の一歩**: 「解けない」から「解けるが難しい」へ切り替えるには第5章が自然です。検証可能性の限界に関心がある読者は、第9章・第12章と行き来しながら読むと応用像が見えます。
## 章末問題
...
迷ったら次の表の順で進め、詰まったら「主に戻る節」にある還元の作法・定理の適用条件・反例を先に見直してください。

| 区分 | 推奨順 | 主に戻る節 | 使い方 |
| --- | --- | --- | --- |
| 基礎問題 | 1番目 | 4.1, 4.2 | 決定可能性と還元の基本判定を確認する |
| 発展問題 | 2番目 | 4.2〜4.6 | 還元構成と階層・相対計算可能性を詰める |
| 探究課題 | 3番目 | 4.1〜4.6 | 歴史・拡張計算モデル・哲学的含意を比較する |
| 実装課題 | 4番目 | 4.1, 4.2, 4.5 | 判定器と還元器を分けて小さく試す |
