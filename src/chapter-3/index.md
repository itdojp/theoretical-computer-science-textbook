...
**定義 3.5** 言語 L₁, L₂ ⊆ \\(\Sigma^{\ast}\\) に対する演算は次のとおりである。
{: #def-3-5 }
...
*証明*（共通部分の場合）：
L₁ を認識する DFA \\(M_1 = (Q_1, \\Sigma, \\delta_1, q_1, F_1)\\)
L₂ を認識する DFA \\(M_2 = (Q_2, \\Sigma, \\delta_2, q_2, F_2)\\)
...
x ≡ₘ y ⟺ \\(\\hat{\\delta}(q_0, x) = \\hat{\\delta}(q_0, y)\\) と定義すると、≡ₘ は有限個の同値類を持つ。
