...
任意のx ∈ ℝに対して\\(f(x) = x^2 \\ge 0\\)なので、例えば-1はfの値域に含まれない。
よってfは全射ではない。□
...
【主張】\\(L_3 = \\{\\,w \\in \\{0,1\\}^{\ast} \\mid \\mathrm{val}_2(w) \\equiv 0 \\pmod{5}\\,\\}\\) の最小DFAは少なくとも 5 状態。

【理由】右同値関係 \\(\\equiv_L\\) で、接尾辞の付加により 5 つの剰余類（mod 5）の右コンテキストが相互に区別可能である。
従って同値類の個数が 5 以上なので、最小DFAの状態数も 5 以上である（実際に 5 状態で実現可能）。□

### 補足: 最小DFAの下界（\\(L_4\\): mod 7）

【主張】\\(L_4 = \\{\\,w \\in \\{0,1\\}^{\ast} \\mid \\mathrm{val}_2(w) \\equiv 0 \\pmod{7}\\,\\}\\) の最小DFAは少なくとも 7 状態。
...
【主張】標準的な 3-SAT から Vertex-Cover への還元で、頂点数・辺数・k は元のインスタンス（変数数 n、節数 m）に対し O(n+m)。
...
**問題**: 章末問題「基礎 3.(f)」について、CLIQUE(G,k) を VERTEX-COVER(G',k') に多項式時間還元せよ。k' の変換（\\(\lvert V\rvert\\) − k）と、サイズ増加が多項式であることを示せ。
...
**部分問題**: opt[i][j] = キー \\(k_i, \\ldots, k_j\\) から構成される最適二分探索木のコスト
...
反復で y'=y*x, x'=x-1 なので y'*(x')! = y*x*(x-1)! = y*x! = n! が保たれる。  
...
(b)
\\(H(X) = -∑_{k\\ge 1} (1-p)^{k-1}p [log p + (k-1)log(1-p)]\\)  
= -log p - E[k-1]·log(1-p)  
...
- 大数の法則により (1/n)∑ -log p(X_i) は H(X) に収束する。
...
特殊健全性：同一コミットメント a に対し、異なるチャレンジ e≠e' で受理される2つの転写
(a,e,z),(a,e',z') から証人 w を効率的に計算できる性質。
