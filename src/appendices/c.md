...
- 付録内の「練習問題X.Y」は、第X章から採録した代表解答番号です。章末問題の全項目番号と一対一に対応しない場合があります。
- 章末問題の特定項目に対応する場合は、問題文中で「章末問題」や該当する基礎・発展・探究・実装区分を明示します。
...
(a) fは単射ではない
(b) fは全射ではない
(c) \\(f\\) を定義域・値域ともに \\(\\mathbb{R}_{\\ge 0}\\) 上に制限すれば全単射である
...
**証明**: 定義域・値域を \\(\\mathbb{R}_{\\ge 0}\\) に制限した \\(f(x) = x^2\\) について：

*単射性*: \\(x_1, x_2 \\in \\mathbb{R}_{\\ge 0}\\) とし、\\(f(x_1) = f(x_2)\\) とする。
\\(x_1^2 = x_2^2\\)であり、\\(x_1, x_2 \\ge 0\\)なので、\\(x_1 = x_2\\)。よって単射。

*全射性*: 任意の \\(y \\in \\mathbb{R}_{\\ge 0}\\) に対して、\\(x = \\sqrt{y}\\) とおけば同じく非負実数であり、
...
よって \\(x_i \\not\\equiv_L x_j\\)（右同値類が異なる）。したがって \\(\\{x_0, x_1, x_2, \\ldots\\}\\) は互いに異なる右同値類に属し、同値類の数は無限個である。
...
**問題**: 章末問題「基礎 2.(c)」T(n) = T(n/3) + T(2n/3) + n を置換法と再帰木で解き、T(n) = Θ(n log n) を示せ（T(1) = Θ(1) とする）。
...
であり、括弧内は負の定数（< 0）である。よってある定数 d > 0 が存在して

\\(T(n) \\le c·n log n − c·d·n + n\\)

となる。c を十分大きく（例: \\(c \\ge 1/d\\)）取れば、右辺は c·n log n 以下に抑えられ、帰納が閉じる。

下界は再帰木から得られる。まず床・天井による定数以下の丸め誤差を漸近記法へ吸収して考える。最初の \\(\\lfloor\\log_3 n\\rfloor\\) レベルではすべての部分問題が基底に達しておらず、各レベルに現れる部分問題サイズの総和は \\(\\Theta(n)\\) である。したがって各レベルの非再帰仕事が \\(\\Theta(n)\\) となり、合計は \\(\\Omega(n\\log n)\\) である。上界と合わせて \\(T(n)=\\Theta(n\\log n)\\)。□
...
| 小問 | 基本形の適用 | 使用する方法 | 漸近解 |
| --- | --- | --- | --- |
| (a) \\(3T(n/4)+n\\log n\\) | 可（ケース3） | マスター定理。通常性条件も成り立つ | \\(\\Theta(n\\log n)\\) |
| (b) \\(2T(n/2)+n\\log n\\) | この章で述べた3ケースには該当しない | 再帰木 | \\(\\Theta(n\\log^2 n)\\) |
| (c) \\(T(n/3)+T(2n/3)+n\\) | 不可（不均等分割） | 置換法と再帰木 | \\(\\Theta(n\\log n)\\) |
| (d) \\(3T(n/2)+n\\) | 可（ケース1） | マスター定理、置換法で照合可能 | \\(\\Theta(n^{\\log_2 3})\\) |
| (e) \\(4T(n/2)+n^2\\) | 可（ケース2） | マスター定理 | \\(\\Theta(n^2\\log n)\\) |
| (f) \\(T(n/2)+n/\\log n\\) | 可（ケース3） | マスター定理 | \\(\\Theta(n/\\log n)\\) |

(b) の再帰木では、深さ \\(i\\) の非再帰仕事は \\(n(\\log n-i\\log 2)\\) である。深さ \\(\\Theta(\\log n)\\) まで和を取ると \\(\\Theta(n\\log^2 n)\\) となる。

(f) では \\(n^{\\log_2 1}=1\\) に対して \\(f(n)=n/\\log n\\) は多項式的に大きい。また、

\\[
\\frac{f(n/2)}{f(n)}
=\\frac{1}{2}\\frac{\\log n}{\\log n-\\log 2}
\\longrightarrow \\frac{1}{2}
\\]

なので、十分大きな \\(n\\) でケース3の通常性条件を満たす。したがって結果は \\(\\Theta(n/\\log n)\\) であり、\\(\\Theta(n)\\) ではない。
