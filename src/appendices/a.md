...
### Markdown上の注意（縦棒・減算記号）

- Markdownの表では `|` が区切り文字になるため、集合の濃度・長さのような `|A|` は `\\|A\\|` のようにエスケープして記述します。
- 本書では、減算は原則として `−`（U+2212）を用い、両側に半角スペースを入れます（例: `|V| − 1`）。コードや疑似コード内では `-` を用います。
