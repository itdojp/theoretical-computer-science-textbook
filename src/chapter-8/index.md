...
**通し例: 安全なメッセージ配送基盤**
配送ノードと回線をグラフとして見ると、最短路は遅延の見積り、最大流は同時配送能力、マッチングはワーカー割当ての近縁問題として読めます。本章では、通し例をネットワーク構造として再解釈します。
...
{: #def-8-1 }
- 無向グラフ：E は V の2要素部分集合（順序を持たない対）の集合である（自己ループを含まない単純グラフの場合）。
- 有向グラフ：E ⊆ V × V

本章の基本アルゴリズムでは、とくに断りがなければ有限グラフを扱う。自己ループ・多重辺・負重み・容量0の辺を許すかどうかは、各アルゴリズムの前提条件として明示する。
...
上の擬似コードは始点 s から到達可能な部分を探索する。グラフ全体の DFS 森が必要な場合は、未訪問頂点ごとに `DFS-Visit` を追加で呼び出す。
...
**定理 8.3** BFS は、単位重みの有向または無向グラフで、始点からの最短路（辺数）を正しく計算する。
{: #thm-8-3 }
...
**定理 8.5** Dijkstraのアルゴリズムは、すべての辺重みが非負であるグラフで正しく単一始点最短路を計算する。
{: #thm-8-5 }
...
動的計画法による解法。負辺は扱えるが、到達可能な負閉路がある場合は最短距離が定義できないため、負閉路の有無を別途確認する。
...
最小全域木問題では、さらに各辺に重み \\(w:E\\to\\mathbb{R}\\) が与えられていると仮定し、全域木の辺重み和を最小化する。

**カット性質**：グラフの任意のカット (S, V \ S) を横切る最小重み辺は、
ある最小全域木に含まれる。

**サイクル性質**：グラフの任意のサイクル中で一意に最大重みを持つ辺は、
どの最小全域木にも含まれない。重みが同値の場合は「最大重み辺」と言うだけでは除外できない。
...
{: #def-8-4 }
- 容量関数 \\(c: V \\times V \\to \\mathbb{R}_{\\ge 0}\\)（辺がない組の容量は 0 とみなす）
- 始点 s、終点 t
...
フローの値は \\(\\lvert f\\rvert = \\sum_v f(s,v) - \\sum_v f(v,s)\\) と定義する。残余グラフでは、前向き辺の残余容量と、流量を戻すための後ろ向き辺を扱う。
...
容量が整数なら各増加でフロー値が少なくとも1増えるため停止する。一般の実数容量では増加路の選び方によって停止性・計算量が問題になるため、Edmonds-Karp 法のように増加路選択を固定した変種を用いる。
...
#### 二部グラフの場合（Hopcroft-Karp 法）
最大濃度マッチングは Hopcroft-Karp 法により \\(O(\\lvert E\\rvert \\sqrt{\\lvert V\\rvert})\\) で解ける。重み付き割当問題で用いられる Hungarian 法とは対象が異なる。
...
**定理 8.12** 提案側を男性とし、全員が相手側全員への厳密な選好順序を持つ標準設定では、Gale-Shapleyアルゴリズムは安定マッチングを生成し、
{: #thm-8-12 }
それは男性最適・女性最悪である。
...
**定理 8.13**（Brooksの定理）
{: #thm-8-13 }
有限単純連結グラフ G が完全グラフでも奇サイクルでもなければ、
χ(G) ≤ Δ(G)（Δ(G) は最大次数）
...
{: #thm-8-17 }
ただし、その順序を効率よく見つける問題は一般には容易でない。任意順序の貪欲彩色は高々 Δ(G)+1 色を使う保証にとどまる。
...
**定理 8.19** 有限単純無向グラフ G について、以下は同値：
{: #thm-8-19 }
1. G は木
...
**性質**：
- 完全消去順序が存在
- 一般グラフでは NP困難な多くの問題が、弦グラフに制限すると多項式時間で解ける

## 8.8 ランダムグラフ

### 8.8.1 Erdős-Rényiモデル

**G(n,p)**：n 頂点の単純無向グラフで、各辺が独立に確率 p で存在。
...
グラフでは対象を構造として表現し、その上でアルゴリズムを設計しました。次章ではさらに一段抽象化し、仕様や性質そのものを論理式で表して、検証と自動推論の基盤へ進みます。

補助資料を併用するなら、手を動かす確認は [付録B](../appendices/b/) の第8章節、現実の接続は [付録E](../appendices/e/) の第8章節、理解確認は [付録F](../appendices/f/) の第8章節を参照してください。
...
迷ったら次の表の順で進め、詰まったら「主に戻る節」にあるアルゴリズムの前提条件・典型図・反例を先に見直してください。

| 区分 | 推奨順 | 主に戻る節 | 使い方 |
| --- | --- | --- | --- |
| 基礎問題 | 1番目 | 8.1〜8.5 | 探索・最短路・マッチングの基本形を確認する |
| 発展問題 | 2番目 | 8.4〜8.7 | 双対性・木幅・近似保証を詰める |
| 探究課題 | 3番目 | 8.1〜8.7 | 理論指標と実ネットワーク応用を比較する |
| 実装課題 | 4番目 | 8.2〜8.7 | アルゴリズム本体と分析ツールを分けて実装する |

### 基礎問題

1. <span id="exq-ch8-001"></span>以下のグラフアルゴリズムを実装し、時間複雑度を解析せよ：
   (a) トポロジカルソート
   (b) 2-SAT の多項式時間解法
   (c) 最長パス（DAG）

- **詳細解答**: [付録Cの対応解答](../appendices/c/#ex-sol-ch8-001)

2. <span id="exq-ch8-002"></span>Dijkstraのアルゴリズムが負の重みで失敗する例を構成せよ。

- **詳細解答**: [付録Cの対応解答](../appendices/c/#ex-sol-ch8-002)

3. <span id="exq-ch8-003"></span>最小全域木がユニークとなる必要十分条件を示せ。

- **詳細解答**: [付録Cの対応解答](../appendices/c/#ex-sol-ch8-003)

4. <span id="exq-ch8-004"></span>二部グラフの最大マッチングを最大フロー問題に帰着せよ。

- **詳細解答**: [付録Cの対応解答](../appendices/c/#ex-sol-ch8-004)
...
6. <span id="exq-ch8-006"></span>グラフの木幅（treewidth）について：
   (a) 定義と計算複雑性を述べよ
   (b) 有界木幅グラフで効率的に解ける問題を列挙せよ

- **詳細解答**: [付録Cの対応解答](../appendices/c/#ex-sol-ch8-006)

7. <span id="exq-ch8-007"></span>頂点彩色問題の近似アルゴリズムについて：
   (a) 貪欲法の近似比を証明せよ
   (b) より良い近似アルゴリズムを設計せよ

- **詳細解答**: [付録Cの対応解答](../appendices/c/#ex-sol-ch8-007)

8. <span id="exq-ch8-008"></span>ネットワーク信頼性について：
   (a) 最小カットと辺連結度の関係を示せ
   (b) k-辺連結グラフの判定アルゴリズムを示せ

- **詳細解答**: [付録Cの対応解答](../appendices/c/#ex-sol-ch8-008)

### 探究課題

9. <span id="exq-ch8-009"></span>ソーシャルネットワーク分析で使用されるグラフ指標（中心性、クラスタリング係数など）について調査し、
   計算アルゴリズムを設計せよ。

- **調査ガイド**: [付録Cの対応解答](../appendices/c/#ex-sol-ch8-009)

10. <span id="exq-ch8-010"></span>グラフニューラルネットワーク（GNN）の理論的基礎について調査し、
    表現力の限界を論ぜよ。

- **調査ガイド**: [付録Cの対応解答](../appendices/c/#ex-sol-ch8-010)

11. <span id="exq-ch8-011"></span>量子アルゴリズムによるグラフ問題の高速化について調査し、
    古典アルゴリズムとの比較を行え。

- **調査ガイド**: [付録Cの対応解答](../appendices/c/#ex-sol-ch8-011)

12. <span id="exq-ch8-012"></span>動的グラフアルゴリズム（頂点・辺の追加削除に対応）について調査し、
    静的アルゴリズムからの改善を論ぜよ。

- **調査ガイド**: [付録Cの対応解答](../appendices/c/#ex-sol-ch8-012)

### 実装課題

13. <span id="exq-ch8-013"></span>グラフアルゴリズムライブラリを実装せよ：
    ```python
...
- **参照実装**: [付録Cの対応解答](../appendices/c/#ex-sol-ch8-013)

14. <span id="exq-ch8-014"></span>ネットワーク分析ツールを実装せよ：
    ```python
...
- **参照実装**: [付録Cの対応解答](../appendices/c/#ex-sol-ch8-014)

15. <span id="exq-ch8-015"></span>グラフ問題の性能比較：
...
- **参照実装**: [付録Cの対応解答](../appendices/c/#ex-sol-ch8-015)
