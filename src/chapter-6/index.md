...
up-sweep と down-sweep はそれぞれ \\(\\log_2 N\\) レベルである。レベルごとの反復数を合計すると各フェーズで \\(N-1\\) 回なので、padding と最後の要素ごとの加算を含む作業量は \\(O(N)=O(n)\\) である。各レベル内を並列実行すると依存するレベル数は \\(2\\log_2 N+O(1)\\) であり、スパンは \\(O(\\log n)\\) となる。
