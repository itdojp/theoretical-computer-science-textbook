...
{: #thm-10-1 }
\(0 \le H(X) \le \log_2 \lvert X\rvert\)
...
\(H(X) = E[-\log_2 p(X)] \le -\log_2 E[p(X)] = -\log_2(1/\lvert X\rvert) = \log_2 \lvert X\rvert\) □
...
**生成行列と検査行列**：
- 生成行列 G（k × n）：C = {uG \(\mid\) u ∈ F_q^k}
- 検査行列 H（(n-k) × n）：C = {c ∈ F_q^n \(\mid\) Hc^T = 0}
- GH^T = 0
...
**構成**：評価写像を用いる
C = {(f(α₁), ..., f(αₙ)) \(\mid\) f ∈ F_q[x], deg(f) < k}
